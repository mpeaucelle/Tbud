...
# Sr: Longwave radiation emitted by the leaf (W m-2)
...
Ebalance<-function(Tbud,T_air,P,RH, Sw,Lw,wind,bud_d,abs_sw=0.5,Ebal_only=FALSE){
  ...
  #abs_sw = 0.5 # leaf absorptivity for shortwave radiation
  abs_lw = 0.97 # leaf absorptivity for longwave radiation
  
  ## 1. Estimates energy balance for given environmental conditions
  ## 1.1 Total absorbed radiation in W m^-2 : Okajima et al. (2012)
  # Rabs = Rsw + Rlw 
  # with Rlw =  (La+Lg)/2 for a spherical object Hammer 1985
  ...
  # r = soil albedo
  r = 0.2
  R_abs = abs_sw * (1 + r) * Sw + ((abs_lw * Lw ) + (abs_lw * s *  T_air ^ 4))/2 # should be Tsol
  # R_abs = abs_sw * (1 + r) * Sw + abs_lw * s * (T_sky ^ 4 + T_air ^ 4)
  # Sw and Lw in W m-2

  
  ## 1.2 re-emitted longwave radiation in W m^-2 : Foster and Smith 1986
  # Tbud in K, We keep 2 here, considering that bud lose radiation from two faces (see Landsberg 1974)
  S_r =  s * abs_lw * Tbud ^ 4
  ...
  # P in kPa
  # T in K
  Tv_leaf = Virtual_Temp(Tbud,P)
  Tv_air = Virtual_Temp(T_air,P,RH)
  D_m = D_m0 * (((T_air+Tbud)/2) / 273.15) ^ eT * (101.3246 / P)
  Gr = (1 / T_air) * G * bud_d ^ 3 * abs(Tv_leaf - Tv_air) / D_m ^ 2
  ...
  # Nu for a smooth sphere Monteith 2013
  ...
  ## 1.4 Latent heat flux in W m^-2
  ## Not computed here, could be computed for wet buds....
...
nu_constant<-function(Re,type,T_air,Tbud,surface){
...
Virtual_Temp<-function (temp,P,RH=1){
  # P = 101.3246 # kPa
  # temp in K
  # RH (0-1), assumed to be 100% in the leaf
